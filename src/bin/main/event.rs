...
async fn image(ctx: &Context, ci: &CommandInteraction) -> Result<()> {
    ...
    let (content, files) = {
        ...
        let monodrama::Image { raw, prompt, ext } = img;
        let monodrama::IMetadata { model } = meta;
...
async fn completion(ctx: &Context, ci: &CommandInteraction) -> Result<()> {
    ...
    let mut msgs = ChainedMessages::new(ctx.clone(), msg)
        .map(|m| {
            if m.is_own(ctx) {
                ...
                monodrama::Message::Model { content }
            } else {
                let content = m.content.clone();

                monodrama::Message::User { content }
            }
        })
        .collect::<Vec<_>>()
        .await;
    ...
    use monodrama::Completion as _;
    let result = match modelname {
        Modelname::GPT35Turbo => {
            let token = &crate::envs().openai_api_key;
            let engine = monodrama::OpenAiGPT35Turbo::new(token);

            tokio::time::timeout(TIMEOUT_DURATION, engine.next(&msgs)).await
        }
        Modelname::GPT4Turbo => {
            let token = &crate::envs().openai_api_key;
            let engine = monodrama::OpenAiGPT4Turbo::new(token);

            tokio::time::timeout(TIMEOUT_DURATION, engine.next(&msgs)).await
        }
        Modelname::GeminiPro => {
            let token = &crate::envs().google_ai_api_key;
            let engine = monodrama::GoogleGeminiPro::new(token);
...
impl EventHandler for EvHandler {
    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        tracing::info!(?version, "Running Monodrama");

...
pub fn envs() -> &'static MonodramaEnv {
    static CACHE: std::sync::OnceLock<MonodramaEnv> = std::sync::OnceLock::new();
...
async fn main() {
    ...
    if let Err(cause) = client.start().await {
        return tracing::error!(%cause, "Failed to starting Monodrama!");
    }
}
